...
const CartPage = () => {
  ...
  const handleSelectItem = (itemId) => {
    setCartItems(cartItems.map((item) => (item.id === itemId ? {...item, checked: !item.checked} : item)));
  };
  ...
  const handleApplyOption = () => {
    if (selectedProduct) {
      setCartItems(
        cartItems.map((item) => (item.id === selectedProduct.id ? {...item, quantity: temporaryQuantity} : item))
      );
    }
    handleCloseModal();
  };
        ...
        <div className='cart-list'>
          ...
          {cartItems.map((item) => (
            <div key={item.id} className='cart-list-item'>
              <div className='cart-item-checkbox'>
                <input type='checkbox' checked={item.checked} onChange={() => handleSelectItem(item.id)} />
              </div>
              <div className='cart-item-content'>
                ...
                <div className='cart-item-info'>
                  ...
                  {isMobile && <div className='cart-item-quantity'>수량: {item.quantity}개</div>}
                  {isMobile ? (
                    <div className='cart-item-mobile-bottom'>
                      <div className='cart-item-mobile-price'>{(item.price * item.quantity).toLocaleString()}원</div>
              ...
              {!isMobile && (
                <div className='pc-quantity-control'>
                  <input type='text' value={item.quantity} className='pc-quantity-input' readOnly />
                  <div className='pc-quantity-buttons'>
                    <button className='pc-quantity-up' onClick={() => handleQuantityChange(item.id, 1)}>
                      <span className='pc-quantity-arrow'>▲</span>
                    </button>
                    <button className='pc-quantity-down' onClick={() => handleQuantityChange(item.id, -1)}>
                      <span className='pc-quantity-arrow'>▼</span>
                    </button>
                  </div>
                </div>
              )}
              {!isMobile && <div className='cart-item-price'>{(item.price * item.quantity).toLocaleString()}원</div>}
              <button className='cart-item-delete' onClick={() => handleRemoveItem(item.id)}>
                <IoClose />
              </button>
            </div>
          ))}
        ...
        <div className='cart-modal-overlay'>
          <div className='cart-modal'>
            ...
            <div className='cart-modal-content'>
              <div className='cart-option-group'>
                ...
                <div className='cart-option-select-wrapper'>
                  <select className='cart-option-select' defaultValue={selectedProduct?.size}>
                ...
                {isMobile && (
                  <div className='modal-quantity'>
                    ...
                    <div className='modal-quantity-control'>
                      <button className='modal-quantity-down' onClick={() => handleModalQuantityChange(-1)}>
                        -
                      </button>
                      <input type='number' value={temporaryQuantity} className='modal-quantity-input' readOnly />
                      <button className='modal-quantity-up' onClick={() => handleModalQuantityChange(1)}>
                        +
                      </button>
                    </div>
                  </div>
            ...
            <div className='cart-modal-footer'>
              ...
              <button className='cart-modal-button cart-modal-button-apply' onClick={handleApplyOption}>
                변경
              </button>
            </div>
          </div>
